...
st.set_page_config(page_title="NL → SQL Chatbot (Chinook)", layout="wide")

st.title("🎵 Chinook Database – NL → SQL Chatbot")
...
- Show results as **text or table**
- Display the generated SQL
"""
)

# Sidebar: optional schema viewer
with st.sidebar:
    st.header("Database")
    if st.button("Show DB Schema"):
        try:
            schema = requests.get(f"{BACKEND_URL}/db/schema").json()
...
st.markdown("---")
st.caption(
    "Backend: FastAPI | Database: Chinook (SQLite) | NL → SQL via LLM"
)
